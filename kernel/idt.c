...
#define IDT_DATA_ENTRY(_isr, priv, _table_type, descriptor_idx, gate)			\
	{																			\
		...
		.selector = {															\
			...
			.descriptor_index = (descriptor_idx / 8)							\
		},																		\
		.flags = {																\
			.gate_type = gate,													\
			.storage_segment = 0,												\
...
void idt_add_interrupt_entry(int entry, void *isr)
{
	...
	desc->selector.descriptor_index = __KERNEL_CODE_SEG / 8;
...
void init_idt_descriptor(struct idt_descriptor *desc, const struct idt_data *data)
{
	...
	desc->zero = 0;
	desc->selector.value = data->selector.value;
	desc->flags.value = data->flags.value;
